...
function verifyForAlways(context, prevNode, nextNode, paddingLines) {
    ...
    log(
        `${context.getFilename()}:${nextNode.loc.start.line}:${nextNode.loc.start.column} prev:`,
        // eslint-disable-next-line no-use-before-define
        getNodeTypes(prevNode, context),
        "next:",
        // eslint-disable-next-line no-use-before-define
...
const StatementTypes = {
    ...
    "implicit-return": {
        test: node => node.parent && node.parent.type === "BlockStatement" &&
            node.parent.parent && node.parent.parent.type === "FunctionDeclaration" &&
            node.parent.parent.loc.end.line === node.loc.end.line + 1
    },
...
/**
 * Check and return the list of statements types that matche with this node
 * @param {ASTNode} node The node to check.
 * @param {RuleContext} context The rule context to report.
 * @returns {Array} the name list of statements types that matche
 */
function getNodeTypes(node, context) {
    const sourceCode = context.getSourceCode();

    return Object.keys(StatementTypes).filter(k => k !== "*").filter(key => StatementTypes[key].test(node, sourceCode));
}
    ...
    create(context) {
        ...
        function verify(node) {
            ...
            if (prevNode) {
                ...
                type.verify(context, prevNode, node, paddingLines, sourceCode);
            }
